...
class time_data_import(models.TransientModel):
    ...
    def faculty_notavailable(self, time):
        ...
        for w in faculty:
            ...
            Name.text = fac_name
            time = self.env['op.faculty.not.available.list'].search([('faculty_id', '=', w.id)])
            count = 0
            for t in time:
                if t.monday == 1:
    ...
    def faculty_maxhr_activity_constraint(self, time):
        ...
        for w in faculty:
            if w.max_hr_cont_act > 0:
                week_days = etree.SubElement(time, "ConstraintTeacherActivityTagMaxHoursContinuously")
                weight = etree.SubElement(week_days, "Weight_Percentage")
                weight_per = w.weight_percent
                fac_name = w.name
                if w.middle_name:
    ...
    def faculty_maxbuild_day(self, space):
        ...
        for r in faculty:
            if r.max_building > 0:
                room = etree.SubElement(space, "ConstraintTeacherMaxBuildingChangesPerDay")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.room_weight
                fac_name = r.name
                if r.middle_name:
                ...
                Name = etree.SubElement(room, "Teacher")
                Name.text = fac_name
                building_name = etree.SubElement(room, "Max_Building_Changes_Per_Week")
    ...
    def faculty_maxbuild_week(self, space):
        ...
        for r in faculty:
            if r.max_build_week > 0:
                room = etree.SubElement(space, "ConstraintTeacherMaxBuildingChangesPerWeek")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.room_weight
                weight.text = str(weight_per)
                building_name = etree.SubElement(room, "Max_Building_Changes_Per_Week")
    ...
    def faculty_mingap(self, space):
        ...
        for r in faculty:
            if r.min_gap_build > 0:
                room = etree.SubElement(space, "ConstraintTeacherMinGapsBetweenBuildingChanges")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.room_weight
                fac_name = r.name
                if r.middle_name:
                ...
                Name = etree.SubElement(room, "Teacher")
                Name.text = fac_name
                building_name = etree.SubElement(room, "Min_Gaps_Between_Building_Changes")
    ...
    def faculties_maxbuild(self, space):
        ...
        for r in faculty:
            if r.max_building_change > 0:
                room = etree.SubElement(space, "ConstraintTeachersMaxBuildingChangesPerDay")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.weight_percent
                weight.text = str(weight_per)
                building_name = etree.SubElement(room, "Max_Building_Changes_Per_Day")
    ...
    def faculties_buildweek(self, space):
        ...
        for r in faculty:
            if r.max_building_week > 0:
                room = etree.SubElement(space, "ConstraintTeachersMaxBuildingChangesPerWeek")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.weight_percent
                weight.text = str(weight_per)
                building_name = etree.SubElement(room, "Max_Building_Changes_Per_Week")
    ...
    def faculties_mingaps(self, space):
        ...
        for r in faculty:
            if r.min_gap_building > 0:
                room = etree.SubElement(space, "ConstraintTeachersMinGapsBetweenBuildingChanges")
                weight = etree.SubElement(room, "Weight_Percentage")
                weight_per = r.weight_percent
                weight.text = str(weight_per)
                building_name = etree.SubElement(room, "Min_Gaps_Between_Building_Changes")
