    ...
    def call_url(self):
        ...
        if model.glusr_mobile and model.glusr_mobile_key:
            if model.timestamp_bool == False and model.date1 and model.date2:
                ...
                url = "https://mapi.indiamart.com/wservce/enquiry/listing/GLUSR_MOBILE/%s/GLUSR_MOBILE_KEY/%s/Start_Time/%s/End_Time/%s/" % (
                str(model.glusr_mobile), str(model.glusr_mobile_key), str(d1), str(d2))
                ...
                url = "https://mapi.indiamart.com/wservce/enquiry/listing/GLUSR_MOBILE/%s/GLUSR_MOBILE_KEY/%s/Start_Time/%s/End_Time/%s/" % (
                str(model.glusr_mobile), str(model.glusr_mobile_key), str(t1), str(t2))
                print(url, "-----------")
                response = requests.get(url, params=None)
                print(response.text, "2222222222222222222222")
            else:
                url = "https://mapi.indiamart.com/wservce/enquiry/listing/GLUSR_MOBILE/%s/GLUSR_MOBILE_KEY/%s/" % (
                str(model.glusr_mobile), str(model.glusr_mobile_key))
                response = requests.get(url, params=None)
